...
if __name__ == '__main__':
    ...
    query = sql.SQL(
        "SELECT COALESCE(SUM(Num_Roles), 0)\
        FROM ActorInMovie IN_AIM\
        WHERE IN_AIM.Movie_Name={movie_name} AND IN_AIM.Year={movie_year}").format(
        movie_name=sql.Literal("movie_temp_1"),
        movie_year=sql.Literal("2000")
    )
    conn.execute("SELECT * FROM MovieTotalNumRoles", printSchema=True)
    ...
    assert getExclusiveActors(True) == [(3, 2)]
